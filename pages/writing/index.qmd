...
For instance, the `release` channel (installed by default) will always point to the current stable version, and the `lts` channel will always point to the long-term support version.
...
The best IDE for Julia is [Visual Studio Code](https://code.visualstudio.com/), or VSCode, developed by Microsoft from an open-source foundation.
...
A more modern language server called [JETLS.jl](https://github.com/aviatesk/JETLS.jl) is currently under development, along with its VSCode extension [`jetls-client`](https://marketplace.visualstudio.com/items?itemName=aviatesk.jetls-client).
Feel free to try it but expect rougher edges.
...
Jupyter notebooks can be opened, modified and run directly from the VSCode editor.
...
[Quarto](https://quarto.org/) is an open-source scientific and technical publishing system, which defines a Markdown variant with the `.qmd` extension.
Quarto markdown files, like standard Markdown files, also integrate with editors such as VSCode -- install the Quarto [extension](https://marketplace.visualstudio.com/items?itemName=quarto.quarto) for a streamlined experience.

Unlike standard markdown files however, Quarto markdown files have executable code chunks, which makes them an alternative to notebooks.
They also give the user control over output and styling, via the YAML header at the top of a `.qmd` file.
Here's an example:
...
As we have seen, Pkg.jl is the Julia equivalent of `pip` or `uv` for Python.
...
If you haven't activated any local project, packages will be installed in the default environment, called `@v1.XX` after the active version of Julia (note the `@` before the name).
...
It is recommended to keep the default environment very light to avoid dependency conflicts: put only essential development tools in there. 
Then, for each of your projects, create a separate local environment to contain the dependencies specific to that project.
...
It is the heart of your package, and will typically look like this once you're done:
...
Most Julia developers also have a [startup file](https://docs.julialang.org/en/v1/manual/command-line-interface/#Startup-file) which is run automatically every time the language is started.
...
Note that `@debug` messages are suppressed by default.
...
The following two packages solve this issue, and they probably belong in your default environment `@v1.XX`, like Revise.jl.
