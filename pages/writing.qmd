---
title: "Writing your code"
---

## Getting help

- [Help page](https://julialang.org/about/help/)
- [Community resources](https://julialang.org/community/)
- [Discourse forum](https://discourse.julialang.org/)
- [Zulip](https://julialang.zulipchat.com/register/)
- [Slack](https://julialang.org/slack/)
- [StartHere.jl](https://github.com/JuliaCommunity/StartHere.jl)

## Installation

- [Julia downloads page](https://julialang.org/downloads/)
- [juliaup](https://github.com/JuliaLang/juliaup)
- [Windows Store](https://www.microsoft.com/store/apps/9NJNWW8PVKMN)
- [current stable version](https://julialang.org/downloads/#current_stable_release) / [long-term support version](https://julialang.org/downloads/#long_term_support_release)

## REPL & Pkg

- [REPL documentation](https://docs.julialang.org/en/v1/stdlib/REPL/)
- [Pkg.jl](https://github.com/JuliaLang/Pkg.jl)
- [Pkg documentation](https://pkgdocs.julialang.org/v1/getting-started/)

## Editor

- [Visual Studio Code](https://code.visualstudio.com/)
- [Julia VSCode extension](https://www.julia-vscode.org/)
- [documentation](https://www.julia-vscode.org/docs/stable/)
- [keyboard shortcuts](https://www.julia-vscode.org/docs/stable/userguide/keybindings/)
- [VSCodium](https://vscodium.com/)
- [Emacs](https://www.gnu.org/software/emacs/)
- [Vim](https://www.vim.org/)
- [JuliaEditorSupport](https://github.com/JuliaEditorSupport)
- [JuliaMono](https://juliamono.netlify.app/)

## Running code

- [Revise.jl](https://timholy.github.io/Revise.jl/stable/user_reference/#Revise.includet)

## Notebooks

- [Jupyter](https://jupyter.org/)
- [IJulia.jl](https://github.com/JuliaLang/IJulia.jl)
- [Pluto.jl](https://plutojl.org/)
- [CodeCells.jl](https://github.com/MasonProtter/CodeCells.jl)

## Markdown

- [Markdown](https://www.markdownguide.org/)

### Quarto

- [Quarto](https://quarto.org/)
- [extension](https://marketplace.visualstudio.com/items?itemName=quarto.quarto)
- [documentation](https://quarto.org/docs/blog/posts/2024-07-11-1.5-release/#native-julia-engine)

## Environments

- [install packages](https://pkgdocs.julialang.org/v1/managing-packages/)
- [manage environments](https://pkgdocs.julialang.org/v1/environments/)
- [environment stacking](https://docs.julialang.org/en/v1/manual/code-loading/#Environment-stacks)
- [environment](https://www.julia-vscode.org/docs/stable/userguide/env/)
- [PkgDependency.jl](https://github.com/peng1999/PkgDependency.jl)

## Local packages

- [create a package](https://pkgdocs.julialang.org/v1/creating-packages/)
- [module](https://docs.julialang.org/en/v1/manual/modules/)

## Development workflow

- [Revise.jl](https://github.com/timholy/Revise.jl)
- [documentation](https://timholy.github.io/Revise.jl/stable/)
- [shared](https://pkgdocs.julialang.org/v1/environments/#Shared-environments)
- [stacked](https://docs.julialang.org/en/v1/manual/code-loading/#Environment-stacks)
- [ShareAdd.jl](https://github.com/Eben60/ShareAdd.jl)

## Configuration

- [startup flags](https://docs.julialang.org/en/v1/manual/command-line-interface/#command-line-interface)
- [startup file](https://docs.julialang.org/en/v1/manual/command-line-interface/#Startup-file)
- [OhMyREPL.jl](https://github.com/KristofferC/OhMyREPL.jl)
- [StartupCustomizer.jl](https://github.com/abraemer/StartupCustomizer.jl)
- [AbbreviatedStackTraces.jl](https://github.com/BioTurboNick/AbbreviatedStackTraces.jl)
- [interactions with VSCode](https://github.com/BioTurboNick/AbbreviatedStackTraces.jl/issues/38)
- [Term.jl](https://github.com/FedeClaudi/Term.jl)
- [advanced configuration](https://fedeclaudi.github.io/Term.jl/stable/adv/adv/)

## Interactivity

- [InteractiveUtils.jl](https://docs.julialang.org/en/v1/stdlib/InteractiveUtils/)
- [About.jl](https://github.com/tecosaur/About.jl)
- [InteractiveCodeSearch.jl](https://github.com/tkf/InteractiveCodeSearch.jl)
- [InteractiveErrors.jl](https://github.com/MichaelHatherly/InteractiveErrors.jl)
- [CodeTracking.jl](https://github.com/timholy/CodeTracking.jl)

## Logging

- [documentation](https://docs.julialang.org/en/v1/stdlib/Logging/)
- [ProgressLogging.jl](https://github.com/JuliaLogging/ProgressLogging.jl)
- [Suppressor.jl](https://github.com/JuliaIO/Suppressor.jl)

## Debugging

### Setting

- [Fermat number](https://en.wikipedia.org/wiki/Fermat_number)

### Infiltrator.jl

- [Infiltrator.jl](https://github.com/JuliaDebug/Infiltrator.jl)
- [integer overflow](https://docs.julialang.org/en/v1/manual/faq/#faq-integer-arithmetic)

### Debugger.jl

- [Debugger.jl](https://github.com/JuliaDebug/Debugger.jl)
- [custom navigation commands](https://github.com/JuliaDebug/Debugger.jl#debugger-commands)
- [graphical interface for debugging](https://www.julia-vscode.org/docs/stable/userguide/debugging/)
