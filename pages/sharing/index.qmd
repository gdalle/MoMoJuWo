...
GitHub is a platform for collaborative software development, based on the version control system called [Git](https://git-scm.com/).
...
Indeed, you can leverage [BestieTemplate.jl](https://github.com/JuliaBesties/BestieTemplate.jl) to automate package creation (like `]generate` from Pkg.jl but on steroids).
The same package also lets you update an existing package to follow evolving best practices by adding or modifying the necessary files.
...
However you obtain it, simply push this new folder to the remote repository <https://github.com/myuser/MyAwesomePackage.jl> (as BestieTemplate.jl instructs you to do), and you're ready to go.
The rest of this post will explain to you what each part of this big scary folder does, and how to bend them to your will.
...
An older alternative to BestieTemplate.jl is [PkgTemplates.jl](https://github.com/JuliaCI/PkgTemplates.jl).
It is less feature-rich and does not allow updates of an existing project, so we no longer recommend it.
Still, if you want to try it out, there is a graphical wrapper called [PackageMaker.jl](https://github.com/Eben60/PackageMaker.jl).

</details>

## GitHub Actions

The most useful aspect of BestieTemplate.jl is that it automatically generates workflows for [GitHub Actions](https://docs.github.com/en/actions/quickstart).
...
For instance, the file `Test.yml` contains instructions that execute the tests of your package (see below) for each pull request, tag or push to the `main` branch, while the file `Docs.yml` builds the documentation.
